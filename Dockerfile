...
RUN wget -c http://sourceforge.net/projects/leanote-bin/files/2.6.1/leanote-linux-amd64-v2.6.1.bin.tar.gz/download -O - | tar -xz --strip 1
COPY run.sh ./run.sh
RUN chmod a+x ./run.sh ./bin/run.sh
